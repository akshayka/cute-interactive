...
def _(Layout, idx2crd, size):
    ...
    def inner_product(coord, stride):
        ...
        if isinstance(_coord, int) and isinstance(stride, int):
            return _coord * stride
        assert len(_coord) == len(stride)
        return sum((inner_product(_c, d) for _c, d in zip(_coord, stride)))
    shape = (4, 8)
    stride = (1, 4)
...
def _(idx2crd):
    ...
    def inner_product_general(coord, stride):
        ...
        if isinstance(_coord, int) and (not isinstance(stride, tuple)):
            return scale(_coord, stride)
        if isinstance(_coord, int) and isinstance(stride, tuple):
            return scale(_coord, stride)
        _result = inner_product_general(_coord[0], stride[0])
        for _c, d in zip(_coord[1:], stride[1:]):
